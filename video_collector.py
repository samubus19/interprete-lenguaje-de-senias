...
def main():
    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        while cap.isOpened():
            ...
            if not is_recording:
                if hands_detected:
                    if wait_counter < WAIT_FRAMES:
                        status = f"Preparando... ({wait_counter}/{WAIT_FRAMES})"
                        wait_counter += 1
                    else:
                        is_recording = True
                        status = "Capturando..."
            ...
            if "Capturando..." in status:
                bg_color = (90, 200, 0)  # Verde
            elif "Preparando..." in status:
            ...
            cv2.putText(image, f"Frase: {phrase}", (10, 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0) if "Preparando" in status else (255, 255, 255), 1, cv2.LINE_AA)
